...
To call this script in the old syntax conkyrc, in before-TEXT section:
    lua_load /path/to/the/lua/script/text.lua
    lua_draw_hook_pre draw_text
 
In the new syntax conkyrc, in before-TEXT section:
    lua_load = '/path/to/the/lua/script/text.lua',
    lua_draw_hook_pre = 'draw_text',
...
function conky_draw_text()
    ...
    text_settings={
		...
		
		{
			...
			reflection_length=0,
			orientation="ww",
			colour={{0.3,col0,0.5},{0.8,col0,0.05}}
		},
		{
			text=conky_parse('${execi 60 ~/.config/conky/Fuyue/scripts/ds.sh}'),
			font_name="Jost",
			font_size=15,
			h_align="l",
			bold=true,
